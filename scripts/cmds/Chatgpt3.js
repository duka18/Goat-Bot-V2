...
module.exports = {
  config: {
    ...
    guide: {
      en: '\nGpt8 < questions >\n\n▀▄▀▄▀▄🔎𝐆𝐩𝐭8 𝗚𝘂𝗶𝗱𝗲▄▀▄▀▄▀\nGpt8 what is capital of France?',
    },
  },
  ...
  onChat: async function ({ api, event, args, getLang, message }) {
    try {
      ...
      if (event.type === 'message_reply') {
        ...
        prompt = event.body.substring(prefix.length).trim() || 'david';
      } else {
        return;
      }

      if (prompt === 'david') {
        const greetingMessage = `${getLang("header")}\n✌✌(•ิ‿•ิ)✌✌ 𝒀𝒐🫡 𝒉𝒖𝒎𝒂𝒊𝒏(𝒆).🥴 David ✔. 𝑩𝒂𝒍𝒂𝒏𝒄𝒆 𝒕𝒐𝒏 𝒑𝒓𝒐𝒃𝒍è𝒎𝒆🧐, 𝒋𝒆 𝒔𝒖𝒊𝒔 𝒍𝒆 𝒔𝒆𝒖𝒍 à 𝒑𝒐𝒖𝒗𝒐𝒊𝒓 𝒕'𝒂𝒊𝒅𝒆𝒓 𝒆𝒏 3𝒔⏳🛌🪅 ✨✌(•ิ‿•ิ)✌\n${getLang("footer")}`;
